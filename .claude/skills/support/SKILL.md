...
```bash
uv run python -c "from config import get_secret; print(get_secret('GH_TEST_ISSUE_REPO'))"
```
...
- Determine the GitHub repository to search/file issues in by fetching the `GH_TEST_ISSUE_REPO` environment variable at runtime (see step 4) — never hardcode a repo name
